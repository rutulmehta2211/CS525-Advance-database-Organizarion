...
// here for the frame list we are creating a queue
...
//Here for the frame list we are dequeing
RC deQueue()
{
...
//here we are enqueing the page frame
RC Enqueue(BM_PageHandle *const page, const PageNumber pageNum, BM_BufferPool *const bm)
{
	...
	if (flag == 0)
	{
		...
		int pageDelete = -1;
		if (queue->filledframes == queue->totalNumOfFrames)
		{ // Removing the page if frames are full
			pageDelete = deQueue();
		}
...
//pins the page with page number using LRU replacement strategy
...
//pins the page with page number using FIFO replacement strategy
...
//by making use of page replacement strategy, creates a new buffer pool with numPages page frames
...
//causes all dirty pages from buffer pool to be written to disk.
...
//write current content of page back to the page file on disk
...
//returns an array of PageNumbers
...
//returns the number of pages that have been written to page file since the initialization of buffer pool
int getNumWriteIO(BM_BufferPool *const bm)
{
	return writeIO;
}

//returns the number of pages that have been read to page file since the initialization of buffer pool
